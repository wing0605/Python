...
判断逻辑：
    摩羯座是跨年的日期段，从它开始。选取结束日期+1作为节点，即20，19号是摩羯，20号是水瓶。以此类推，
通过输入的月，来判断应该获取几号位的日来确定星座
'''

sdate = [20, 19, 21, 20, 21, 22, 23, 23, 23, 24, 23, 22]  # 星座判断列表
conts = ['摩羯座', '水瓶座', '双鱼座', '白羊座', '金牛座', '双子座', '巨蟹座', '狮子座', '处女座', '天秤座', '天蝎座', '射手座', '摩羯座']
signs = ['♑', '♒', '♓', '♈', '♉', '♊', '♋', '♌', '♍', '♎', '♏', '♐', '♑']
types = ['务实本分', '作天作地', '安于现状', '积极乐观', '固执内向', '圆滑世故', '多愁善感', '迷之自信', '精明计较', '犹豫不决', '阴暗消极', '放荡不羁', '务实本分']
...
def sign(cmonth, cdate):
    if int(cdate) < sdate[int(cmonth) - 1]:  # 如果日数据早于对应月列表中对应的日期
        print('星座类型： ', conts[int(cmonth) - 1])  # 直接输出星座列表对应月对应的星座名称
        print('星座符号： ', signs[int(cmonth) - 1])  # 直接输出星座列表对应月对应的星座符号
        print('星座特点： ', types[int(cmonth) - 1])  # 星座特点
    else:
        print('星座类型： ', conts[int(cmonth)])  # 否则输出星座列表下一月对应的星座名称
        print('星座符号： ', signs[int(cmonth)])  # 否则输出星座列表下一个对应的星座符号
        print('星座特点： ', types[int(cmonth)])  # 星座特点
